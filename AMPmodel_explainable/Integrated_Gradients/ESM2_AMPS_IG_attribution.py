...
class Dim1ImportanceAnalyzer:
    ...
    def compute_dim1_importance(self, dataloader, baseline=None):
        """
        - dim1_imp: numpy array shape(20,)
        - all_attributions: he original attribution of all samples
        """
        if baseline is None:
            baseline = torch.zeros((1, 20, 1280)).to(self.device)
...
dim1_importance_df.to_excel(save_path + '/ESM2_AMPS_IG.xlsx', index=False)
np.save(os.path.join(save_path, "ESM2_AMPS_attributions.npy"), all_attributions)
...
plt.title('ESM2_AMPS model features mean IG values', fontsize=16)
...
plt.savefig(save_path + '/ESM2_AMPS_model_features_mean_IG_values.pdf', bbox_inches='tight')
plt.show()
