...
from Feature_attribution.cls_segment_eos.AE_DNN.DNN_model import DNN
...
class IGFeatureImportance:
    ...
    def plot_importance(mean_attributions, feature_names, save_path=None):
        ...
        plt.title('cls_segment_eos Integrated Gradients Feature Importance')
...
def main():
    ...
    save_path = os.path.join('Feature_attribution/cls_segment_eos/AE_DNN_IG/output')
    ...
    model = DNN(input_dim=3600, hidden1_dim=1564, hidden2_dim=445, output_dim=1).to(device)
    model.load_state_dict(torch.load('Feature_attribution/cls_segment_eos/train_model/cls_segment_eos_AE_DNN_model.pth'))
    ...
    ig_analyzer.plot_importance(
        ...
        group_names,
        save_path=os.path.join(save_path, 'cls_segment_eos_Integrated_Gradients_Feature_Importance_plot.pdf')
    )
main()