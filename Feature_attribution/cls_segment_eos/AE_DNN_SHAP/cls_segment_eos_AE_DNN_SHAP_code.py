...
def main():
    ...
    save_path = os.path.join('Feature_attribution/cls_segment_eos/AE_DNN_SHAP/output')
    ...
    pd.DataFrame(grouped_attributions, columns=group_names).to_csv(
        os.path.join(save_path, 'cls_segment_eos_AE_DNN_SHAP_values.csv'), index=False)

    shap_analyzer.plot_importance(
        grouped_attributions,
        group_names,
        save_path=os.path.join(save_path, 'cls_segment_eos_AE_DNN_SHAP_plot.pdf')
    )
