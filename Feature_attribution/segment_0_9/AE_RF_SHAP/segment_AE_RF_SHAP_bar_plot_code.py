...
def main():
    ...
    save_path = os.path.join('Feature_attribution/segment_0_9/AE_RF_SHAP/output')
    ...
    # Load the model
    model = load('Feature_attribution/segment_0_9/train_model/seg_best_rf_model.pkl')
    ...
    pd.DataFrame(grouped_vals, columns=group_names).to_csv(
        os.path.join(save_path, 'segment_AE_RF_SHAP_values.csv'), index=False)

    SHAPFeatureImportance_RF.plot_importance(
        grouped_vals,
        group_names,
        save_path=os.path.join(save_path, 'segment_AE_RF_SHAP_plot.pdf')
    )
