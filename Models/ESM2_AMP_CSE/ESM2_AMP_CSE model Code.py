...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, data_features, data_label, reshape_shape=(24, 1280)):
...
model = TransformerMLP(input_dim=1280, hidden1_dim=480, hidden2_dim=240, output_dim=1).float().to(device)


# Load the trained model weights
checkpoint = torch.load("ESM2_AMP_CSE_checkpoint.pth")
...
predictions_df.to_csv(os.path.join(save_path, 'test_predictions_ESM2_AMP_CSE.csv'), index=False)
